...
export default function BlogDashboardPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      blog_hero_title: textData?.blog_hero_title?.value,
      blog_hero_subtitle: textData?.blog_hero_subtitle?.value,
    };
  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div className="mb-8">
        <h2 className="text-2xl font-bold font-poppins text-header">Manajemen Halaman Blog</h2>
        <p className="text-body mt-1">Ubah konten teks hero yang ditampilkan di halaman Blog & Publikasi.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white p-6 rounded-lg border border-gray-100 shadow-sm">
          ...
          <div className="space-y-4">
            <div>
              <label className="block font-inter text-sm font-semibold text-header mb-1">Blog Hero Title</label>
              <input
                type="text"
                className={getInputClass('blog_hero_title')}
                value={textData?.blog_hero_title?.value || ''}
                onChange={(e) => handleTextChange('blog_hero_title', e.target.value)}
              />
              <p className={getCharCountClass('blog_hero_title')}>
                {textData?.blog_hero_title?.value?.length || 0} / {textData?.blog_hero_title?.max_length} karakter
              </p>
            </div>

            <div>
              <label className="block font-inter text-sm font-semibold text-header mb-1">Blog Hero Subtitle</label>
              <textarea
                rows={3}
                className={getInputClass('blog_hero_subtitle')}
                value={textData?.blog_hero_subtitle?.value || ''}
                onChange={(e) => handleTextChange('blog_hero_subtitle', e.target.value)}
              />
              <p className={getCharCountClass('blog_hero_subtitle')}>
                {textData?.blog_hero_subtitle?.value?.length || 0} / {textData?.blog_hero_subtitle?.max_length} karakter
              </p>
            </div>
          </div>
        </div>
